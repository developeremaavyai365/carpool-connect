-- Geospatial ride matching: PostGIS, trips, bookings, driver profiles
-- Idempotent where possible

CREATE EXTENSION IF NOT EXISTS postgis;

-- Driver stats for ranking (users who publish trips)
...
-- Geospatial trip entity (BlaBlaCar-style)
...
-- GIST index for corridor matching (10km proximity queries)
CREATE INDEX IF NOT EXISTS idx_trips_route_geometry_gist
  ON public.trips USING GIST (route_geometry);

CREATE INDEX IF NOT EXISTS idx_trips_active_departure
...
CREATE POLICY trips_select ON public.trips FOR SELECT
  USING (status = 'active' OR driver_id = current_app_user_id());
...
CREATE POLICY bookings_select ON public.bookings FOR SELECT
  USING (passenger_id = current_app_user_id()
    OR trip_id IN (SELECT id FROM public.trips WHERE driver_id = current_app_user_id()));
...
-- Atomic seat booking function (prevents race conditions)
...
BEGIN
  ...
  IF v_trip.driver_id = p_passenger_id THEN
    RAISE EXCEPTION 'Cannot book your own trip';
  END IF;
  ...
  INSERT INTO public.bookings (
    ...
    p_trip_id, p_passenger_id, p_seats,
...
-- PostGIS helpers for matching (used by backend matching service)
COMMENT ON COLUMN public.trips.route_geometry IS 'Full driving route as GEOGRAPHY LINESTRING (WGS84)';
